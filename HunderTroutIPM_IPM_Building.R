...
mO.2yr.pred <- function(size, dam, t, orig, mO1size.factor){
	...
	if(dam == 1){
		...
		y.mO[which(size.std < 0)] <- log(Mu.mO1[orig]) + beta1.mO1*discF.std[t] + beta2.mO1*size.std[which(size.std < 0)]*mO1size.factor + epsilon.mO[t]
...
Sa.pred <- function(size, dam, t, orig, min.size, max.size, spawn.year, l.limit, u.limit, mH.factor, mO1size.factor){ 
		
	# Calculate the vector of survival probabilities (whole size range)
	mTOT <- mH.2yr.pred(size, t, orig, l.limit, u.limit, mH.factor) + mO.2yr.pred(size, dam, t, orig, mO1size.factor)	
	Sa <- exp(-mTOT)

	# Calculate minimum & maximum size survival
	mTOT.min <- (mH.2yr.pred(min.size, t, orig, l.limit, u.limit, mH.factor) + mO.2yr.pred(min.size, dam, t, orig, mO1size.factor))
	mTOT.max <- (mH.2yr.pred(max.size, t, orig, l.limit, u.limit, mH.factor) + mO.2yr.pred(max.size, dam, t, orig, mO1size.factor))
	Sa.min <- exp(-mTOT.min)
	Sa.max <- exp(-mTOT.max)
...
K.AsAn.fun  <-function(t, dam, sex, orig, min.size, max.size, l.limit, u.limit, mH.factor, mO1size.factor){
  ...
  Svec <- Sa.pred(x, dam, t, orig, min.size, max.size, spawn.year = 1, l.limit, u.limit, mH.factor, mO1size.factor) 
...
K.AnAs.fun  <-function(t, dam, sex, orig, min.size, max.size, l.limit, u.limit, mH.factor, mO1size.factor){
  ...
  Svec <- Sa.pred(x, dam, t, orig, min.size, max.size, spawn.year = 0, l.limit, u.limit, mH.factor, mO1size.factor) # Survival is 1 here
...
build.IPM <- function(t, sex, orig, m0, Mu.mj, Mu.mdam, Mu.mOs, beta2.mj, beta2.mdam, beta2.mOs, dPenalty.0, dPenalty.j, l.limit, u.limit, mH.factor, mO1size.factor){
	...
	K.AsAn.u <- K.AsAn.fun(t, 1, sex, orig, Lx, Ux, l.limit, u.limit, mH.factor, mO1size.factor)
	K.AsAn.d <- K.AsAn.fun(t, 0, sex, orig, Lx, Ux, l.limit, u.limit, mH.factor, mO1size.factor)
	K.AnAs.u <- K.AnAs.fun(t, 1, sex, orig, Lx, Ux, l.limit, u.limit, mH.factor, mO1size.factor) # new kernel (+ dam argument)
	K.AnAs.d <- K.AnAs.fun(t, 0, sex, orig, Lx, Ux, l.limit, u.limit, mH.factor, mO1size.factor) # new kernel (+ dam argument)
