...
# (none, +50%, +100%) and runs a sensitivity analysis of population growth rate without stocking.
...
pop.analysis <- function(t, sex, orig, m0, Mu.mj, Mu.mdam, Mu.mOs, beta2.mj, beta2.mdam, beta2.mOs, dPenalty.0, dPenalty.j, dy){
	
	## Build separate kernels
	K.JJ.u <- K.JJ.fun(t, 1, Mu.mj, beta2.mj, dPenalty.j, dy)
	K.JJ.d <- K.JJ.fun(t, 0, Mu.mj, beta2.mj, dPenalty.j, dy)
	K.JS.u <- K.JS.fun(t, orig, 1, Mu.mj, beta2.mj, Mu.mdam, beta2.mdam, dPenalty.j, dy)
	K.JS.d <- K.JS.fun(t, orig, 0, Mu.mj, beta2.mj, Mu.mdam, beta2.mdam, dPenalty.j, dy)
	K.SS <- K.SS.fun(t, sex, orig, Mu.mOs, beta2.mOs, dy)
	K.SAs.u <- K.SAs.fun(t, 1, sex, orig, Mu.mOs, beta2.mOs, dy)
	K.SAs.d <- K.SAs.fun(t, 0, sex, orig, Mu.mOs, beta2.mOs, dy)
	K.AsJ.u <- K.AsJ.fun(t, m0, 1, orig, dPenalty.0, dy)
	K.AsJ.d <- K.AsJ.fun(t, m0, 0, orig, dPenalty.0, dy)
	K.AsAn.u <- K.AsAn.fun(t, 1, sex, orig, Lx, Ux, dy)
	K.AsAn.d <- K.AsAn.fun(t, 0, sex, orig, Lx, Ux, dy)
	K.AnAs.u <- K.AnAs.fun(t, 1, sex, orig, Lx, Ux, dy)
	K.AnAs.d <- K.AnAs.fun(t, 0, sex, orig, Lx, Ux, dy)
	...
	results$perturbation <- ifelse(sum(dy) < 0, names(dy)[which(dy!=0)], 'orig')
	
	return(results)	
}

## Run population analysis (for 3 different penalties)
orig.data1 <- pop.analysis(test.year, 1, 1, m0, Mu.mj, Mu.mdam, Mu.mOs, beta2.mj, beta2.mdam, beta2.mOs, dPenalty.0=1, dPenalty.j=1, dy = dy[,1])

pert.data1 <- do.call("rbind", sapply(2:ncol(dy), FUN = function(z) pop.analysis(test.year, 1, 1, m0, Mu.mj, Mu.mdam, Mu.mOs, beta2.mj, beta2.mdam, beta2.mOs, dPenalty.0=1, dPenalty.j=1, dy = dy[,z]), simplify = FALSE))

orig.data2 <- pop.analysis(test.year, 1, 1, m0, Mu.mj, Mu.mdam, Mu.mOs, beta2.mj, beta2.mdam, beta2.mOs, dPenalty.0=1.5, dPenalty.j=1, dy = dy[,1])

pert.data2 <- do.call("rbind", sapply(2:ncol(dy), FUN = function(z) pop.analysis(test.year, 1, 1, m0, Mu.mj, Mu.mdam, Mu.mOs, beta2.mj, beta2.mdam, beta2.mOs, dPenalty.0=1.5, dPenalty.j=1, dy = dy[,z]), simplify = FALSE))

orig.data3 <- pop.analysis(test.year, 1, 1, m0, Mu.mj, Mu.mdam, Mu.mOs, beta2.mj, beta2.mdam, beta2.mOs, dPenalty.0=2, dPenalty.j=1, dy = dy[,1])

pert.data3 <- do.call("rbind", sapply(2:ncol(dy), FUN = function(z) pop.analysis(test.year, 1, 1, m0, Mu.mj, Mu.mdam, Mu.mOs, beta2.mj, beta2.mdam, beta2.mOs, dPenalty.0=2, dPenalty.j=1, dy = dy[,z]), simplify = FALSE))
...
sens.fun <- function(orig.data, pert.data, VR, dy){
	...
	sens <- (data[,2:5] - orig.data[,2:5])/dy
...
ggplot(subset(test, stage == 'Subadult'), aes(x = perturbation, y = lambda, group = Penalty)) + geom_bar(aes(fill = Penalty), stat = 'identity', position = 'dodge') + ylab(expression('Sensitivity of '~lambda)) + ggtitle('b) Population without stocking') +
scale_fill_manual(name = 'Below-dam penalty', values = c('#73C76D', '#008087', 'black')) +
scale_x_discrete(labels = par.label) + 
